        ...
        while (attempt < 3 && !match)
        {
            ...
            for (int index = 0; index < count; index++)
            {
                tasks[index] = Task.Run(async () =>
                {
                    ...
                    string payload = JsonSerializer.Serialize(new { Status = status, Message = note, Error = new { }, Value = new { ClientOrderNum = RandomNumberGenerator.GetInt32(1, 100_000), NumEDocument = (long)RandomNumberGenerator.GetInt32(1, 100_000), ErrorCode = error, ErrorText = string.Empty }, Extra = "ignored" });
