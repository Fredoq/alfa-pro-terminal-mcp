...
/// <summary>
/// Verifies ClientBalanceEntityResponse retrieves payloads from router stream. Usage example: executed by xUnit runner.
/// </summary>
public sealed class ClientBalanceEntityResponseTests
{
    /// <summary>
    /// Ensures that ClientBalanceEntityResponse returns the first matching payload. Usage example: await response.NextMessage(token).
    /// </summary>
    [Fact(DisplayName = "ClientBalanceEntityResponse returns payload from matching response")]
    public async Task Given_matching_response_when_next_message_then_returns_payload()
    {
        ...
        string payload = JsonSerializer.Serialize(new { Account = account, Name = $"данные-{Guid.NewGuid()}-λ" });
        ...
        Messaging.Responses.ClientBalanceEntityResponse response = new(incoming, socket, logger);
        using CancellationTokenSource source = new(TimeSpan.FromSeconds(2));
        string value = await response.NextMessage(source.Token);
        Assert.True(value == payload, "ClientBalanceEntityResponse does not return matching payload");
    }

    /// <summary>
    /// Confirms that ClientBalanceEntityResponse fails when matching response is absent. Usage example: await response.NextMessage(token).
    /// </summary>
    [Fact(DisplayName = "ClientBalanceEntityResponse throws when no response matches")]
    public async Task Given_no_matching_response_when_next_message_then_throws()
    {
        ...
        string payload = JsonSerializer.Serialize(new { Account = account, Name = $"нет-{Guid.NewGuid()}-μ" });
        string other = Build(Guid.NewGuid().ToString(), payload, "#Data.Query");
