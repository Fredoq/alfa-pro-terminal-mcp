...
/// <summary>
/// Simulates router behavior for limit requests by echoing a crafted response. Usage example: new LimitSocketFake(payload).
/// </summary>
internal sealed class LimitSocketFake : ITerminal
{
    private readonly string _payload;
    private readonly TaskCompletionSource<string> _id;

    /// <summary>
    /// Initializes the fake with response payload. Usage example: new LimitSocketFake(payload).
    /// </summary>
    ...
    public async IAsyncEnumerable<string> Messages([EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        string message = new ResponseText(id, _payload, "#Order.Limit.Query", "response").Value();
